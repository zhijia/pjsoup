...
class HtmlTreeBuilder extends TreeBuilder {
    ...
    @Override
    protected boolean process(Token token, VFlag vFlag) {
        currentToken = token;
        return this.state.process(token, this, vFlag);
    }

    boolean process(Token token, HtmlTreeBuilderState state, VFlag vFlag) {
        currentToken = token;
        return state.process(token, this, vFlag);
    }
    ...
    Element insert(Token.StartTag startTag) {
        ...
        if (startTag.isSelfClosing() && !Tag.isKnownTag(startTag.name())) {
            ...
            process(new Token.EndTag(el.tagName()), new VFlag(true)); // ensure we get out of whatever state we are in
            return el;
        }
